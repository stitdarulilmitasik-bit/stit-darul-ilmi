...
class HomeController extends Controller
{
    ...
    public function adviceStore(Request $request)
    {
        ...
        if($saran->save()){
            ...
            if ($recipient) {
                Mail::send('base.resource.mail-kotak-saran-admin', ['saran' => $saran], function($message) use ($saran, $recipient) {
                    $message->to($recipient);
                    $message->subject('[ SARAN ] - STIT Darul Ilmi - ' . $saran->subject);
                    $message->from(config('mail.from.address'), config('mail.from.name', 'STIT Darul Ilmi'));
                });
            }
